...
async function fetchPendingConnections(): Promise<void> {
  ...
  if (result.status !== 200) {
    notifyError("Error getting connections: " + result.body.message);
...
async function fetchFriends(): Promise<void> {
  ...
  if (result.status !== 200) {
    notifyError("Error getting friends: " + result.body.message);
...
qs(".pageFriends .pendingRequests table")?.on("click", async (e) => {
  ...
  if (result.status !== 200) {
    notifyError(`Cannot update friend request: ${result.body.message}`);
  } else {
    //remove from cache
