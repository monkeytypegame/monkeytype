...
export default class Quotes {
  ...
  async isSubmissionEnabled(): Ape.Endpoint {
  ...
  async rejectSubmission(quoteSubmissionId: string): Ape.Endpoint {
    return await this.httpClient.post(`${BASE_PATH}/reject`, {
      payload: { quoteId: quoteSubmissionId },
    });
  }

  async getRating(quote: MonkeyTypes.Quote): Ape.Endpoint {
    const searchQuery = {
      quoteId: quote.id,
  ...
  async addRating(quote: MonkeyTypes.Quote, rating: number): Ape.Endpoint {
    const payload = {
      quoteId: quote.id,
      rating,
      language: quote.language,
