...
function PuncAndNum(): JSXElement {
  ...
  const meta = configMetadata as ConfigMetadataObject;
      ...
      <AnimeShow when={getConfig.mode !== "zen"} duration={durationMs}>
        <div class={cardClass}>
          <For each={buttons}>
            {(configKey) => (
              <TCButton
                fa={meta[configKey].fa}
                text={meta[configKey].displayString ?? configKey}
      ...
      <For each={modeOptions}>
        {(modeOption) => (
          <TCButton
            ...
            text={
              modeMeta.optionsMetadata?.[modeOption]?.displayString ??
              modeOption
            }
