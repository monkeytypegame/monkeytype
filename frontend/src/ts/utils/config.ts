...
export function replaceLegacyValues(configObj: PartialConfig): PartialConfig {
  //@ts-expect-error
  ...
  if (configObj.showLiveWpm === true) {
    let val: LiveSpeedAccBurstStyle = "mini";
    if (configObj.timerStyle !== "bar" && configObj.timerStyle !== "off") {
      val = configObj.timerStyle as LiveSpeedAccBurstStyle;
    }
