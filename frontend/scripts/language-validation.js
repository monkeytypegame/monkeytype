...
const DUPLICATE_PERCENTAGE_QUALITY_GATE = 10;
...
function validateLanguageContents(languageNameFilter) {
  return new Promise((resolve, reject) => {
    const languagesData = JSON.parse(
      ...
      (language) =>
        languageNameFilter === undefined ||
        language.startsWith(languageNameFilter)
    );
    ...
    languagesData.forEach((language) => {
      ...
      let duplicatePercentage = Math.round(
        (duplicates.length / languageFileData.words.length) * 100
      );
      if (duplicatePercentage > DUPLICATE_PERCENTAGE_QUALITY_GATE) {
        console.warn(
          `Language '${languageFileData.name}' may contain ${duplicates.length} (${duplicatePercentage}%) duplicate(s):`
        );
    ...
    if (!allLanguageFilesOk) {
      return reject(
        new Error(
          `Detected ${results.length} languages with duplicate percentage over ${DUPLICATE_PERCENTAGE_QUALITY_GATE}% !`
        )
      );
    }
