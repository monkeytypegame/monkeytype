...
const RECENT_AGE_MILLISECONDS = 10 * 60 * 1000;

async function getTop10(leaderboardTime: string): Promise<SharedTypes.LeaderboardEntry[]> {
  return (await LeaderboardsDAL.get("time", leaderboardTime, "english", 0, 10)) as SharedTypes.LeaderboardEntry[];
}

async function updateLeaderboardAndNotifyChanges(leaderboardTime: string): Promise<void> {
  const top10BeforeUpdate = await getTop10(leaderboardTime);
  const previousRecordsMap = Object.fromEntries(top10BeforeUpdate.map((record) => [record.uid, record]));
  ...
  const newRecords = top10AfterUpdate.filter((record) => {
    ...
    const userImprovedRank = previousRecordsMap[userId] && previousRecordsMap[userId].rank > record.rank;
    const newUserInTop10 = !(userId in previousRecordsMap);
    const isRecentRecord = record.timestamp > Date.now() - RECENT_AGE_MILLISECONDS;
