...
const collectionsToLog = ["ape-keys", "configs", "errors", "logs", "presets", "reports", "results", "users"];

async function main(): Promise<void> { await Promise.all(collectionsToLog.map(set)); }
