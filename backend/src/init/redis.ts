...
export async function connect(): Promise<void> {
  ...
  if (!(REDIS_URI ?? "")) {
    if (isDevEnvironment()) { Logger.warning("No redis configuration provided. Running without redis."); return; }
    throw new Error("No redis configuration provided");
  }
  connection = new IORedis(REDIS_URI, { maxRetriesPerRequest: null, enableReadyCheck: false, lazyConnect: true });
  try {
    ...
    if (isDevEnvironment()) { await connection.quit(); Logger.warning(`Failed to connect to redis. Continuing in dev mode, running without redis.`); }
    else { Logger.error("Failed to connect to redis. Exiting with exit status code 1."); process.exit(1); }
  }
}
...
export function getConnection(): IORedis.Redis | undefined {
  ...
  if (connection === undefined || status !== "ready") { return undefined; }
  return connection;
}
