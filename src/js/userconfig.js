...
//mode
function setTimeConfig(time, nosave, mp = false) {
  if (!mp_checkIfCanChangeConfig(mp)) {
    return;
  }
...
//quote length
function setQuoteLength(len, nosave, mp = false) {
  if (!mp_checkIfCanChangeConfig(mp)) {
    return;
  }
  ...
  len = parseInt(len);
  if (!nosave) setMode("quote", nosave, mp);
...
function setWordCount(wordCount, nosave, mp = false) {
  if (!mp_checkIfCanChangeConfig(mp)) {
    return;
  }
...
function setLanguage(language, nosave, mp = false) {
  if (!mp_checkIfCanChangeConfig(mp)) {
    return;
  }
